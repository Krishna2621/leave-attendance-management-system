...
const createDepartment = async (req, res) => {
  try {
    ...
    await createHRAuditLog({
      ...
      action: "created",
      actor: req.user,
      changedFields: { name: { before: null, after: department.name } },
    });
    return res
      .status(201)
    ...
    if (err.code === 11000)
      return fail(res, error("A department with this name already exists", 409));
    return fail(res, err);
  }
};
...
const getDepartment = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Department fetched successfully", data: { department } });
  } catch (err) {
    return fail(res, err);
  }
};
...
const updateDepartment = async (req, res) => {
  try {
    ...
    if (req.body.isActive === false) {
      ...
      if (employees)
        throw error(
          "Reassign or deactivate active employees before deactivating this department",
          409
        );
    }
    const before = department.toObject();
    ["name", "description", "managerId", "isActive"].forEach((field) => {
      if (Object.hasOwn(req.body, field)) department[field] = req.body[field];
    });
    ...
    await createHRAuditLog({
      ...
      action: "updated",
      actor: req.user,
      changedFields: changes(before, department, ["name", "description", "managerId", "isActive"]),
    });
    return res
      .status(200)
      .json({ success: true, message: "Department updated successfully", data: { department } });
  } catch (err) {
    if (err.code === 11000)
      return fail(res, error("A department with this name already exists", 409));
    return fail(res, err);
...
const deleteDepartment = async (req, res) => {
  try {
    const department = await Department.findById(req.params.id);
    if (!department) throw error("Department not found", 404);
    if (await User.exists({ departmentId: department._id }))
      throw error("Cannot delete a department that still contains employees", 409);
    ...
    await createHRAuditLog({
      ...
      action: "deleted",
      actor: req.user,
      changedFields: { name: { before: department.name, after: null } },
    });
    return res
      .status(200)
      .json({ success: true, message: "Department deleted successfully", data: {} });
  } catch (err) {
    return fail(res, err);
  }
};
