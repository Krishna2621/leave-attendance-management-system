...
const queueLeaveReminders = async ({ targetDate = getBusinessDate(new Date()) } = {}) => {
  ...
  const requests = await LeaveRequest.find({ status: "approved", startDate })
    .select("_id userId leaveTypeId startDate endDate totalDays")
  ...
  return queueNotifications(
    requests.map((request) => ({
      ...
      payload: {
        leaveTypeName: request.leaveTypeId?.name || "Leave",
        startDate: formatDate(request.startDate),
        endDate: formatDate(request.endDate),
        totalDays: request.totalDays,
      },
      metadata: { source: "cron", reminderDate: runDate },
      dedupeKey: `leave-reminder:${request._id}:${runDate}`,
