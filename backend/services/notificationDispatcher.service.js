  ...
  return Notification.findOneAndUpdate(
    {
      ...
      $or: [{ status: "queued" }, { lockUntil: { $lt: now } }],
    },
    {
      $set: { status: "processing", lockUntil: new Date(now.getTime() + lockMinutes * 60000) },
      $inc: { attemptCount: 1 },
    },
    { new: true }
  );
...
const dispatchOne = async (notification) => {
  const recipient = await User.findOne({ _id: notification.recipientId, isActive: true })
    .select("name email")
    .lean();
  ...
  if (!template)
    throw new Error(`Unsupported email notification template: ${notification.template}`);
...
const dispatchQueuedNotifications = async ({
  limit = Number(process.env.NOTIFICATION_BATCH_SIZE || 50),
} = {}) => {
      ...
      await Notification.updateOne(
        { _id: notification._id },
        {
          $set: {
            ...
            lockUntil: null,
            nextAttemptAt: new Date(Date.now() + retryDelay * 60000),
