...
const markAutomaticAbsences = async ({ targetDate = getPreviousBusinessDate() } = {}) => {
  ...
  if (isNonWorkingDay(businessDate))
    return {
      skipped: true,
      reason: "Target date is a non-working day",
      targetDate: formatDate(businessDate),
      createdCount: 0,
    };
  ...
  if (!employeeIds.length)
    return { skipped: false, targetDate: formatDate(businessDate), createdCount: 0 };
  const existing = await Attendance.find({ userId: { $in: employeeIds }, date: businessDate })
    .select("userId")
    .lean();
...
const queueAttendanceReminders = async ({ targetDate = getBusinessDate(new Date()) } = {}) => {
  const businessDate = typeof targetDate === "string" ? toBusinessDate(targetDate) : targetDate;
  if (isNonWorkingDay(businessDate))
    return { skipped: true, reason: "Target date is a non-working day", queuedCount: 0 };
  const employees = await User.find({ isActive: true }).select("_id").lean();
  const ids = employees.map((employee) => employee._id);
  const existing = await Attendance.find({ userId: { $in: ids }, date: businessDate })
    .select("userId")
    .lean();
  const existingIds = new Set(existing.map((record) => String(record.userId)));
  ...
  return queueNotifications(
    ids
      ...
      .map((recipientId) => ({
        ...
        dedupeKey: `attendance-reminder:${recipientId}:${date}`,
