...
const acquireJobRun = async ({ jobName, runKey, metadata = {} }) => {
  ...
  try {
    return await ScheduledJobRun.create({
      jobName,
      runKey,
      status: "running",
      lockUntil,
  ...
  if (!existing || existing.status === "completed" || existing.attemptCount >= maxAttempts)
    return null;
  return ScheduledJobRun.findOneAndUpdate(
    {
      _id: existing._id,
      $or: [{ status: "failed" }, { status: "running", lockUntil: { $lt: now } }],
    },
    {
      $set: {
        status: "running",
...
const runScheduledJob = async ({ jobName, runKey, metadata, handler }) => {
  ...
  if (!jobRun)
    return { skipped: true, reason: "Job run is already completed, locked, or exhausted retries" };
