...
const runAttendanceAbsenceJob = (targetDate) => {
  const date = resolveDate(targetDate, getPreviousBusinessDate());
  return runScheduledJob({
    jobName: "attendance-absence",
    runKey: dateKey(date),
    metadata: { targetDate: dateKey(date) },
    handler: () => markAutomaticAbsences({ targetDate: date }),
  });
};
const runAttendanceReminderJob = (targetDate) => {
  const date = resolveDate(targetDate, getBusinessDate(new Date()));
  return runScheduledJob({
    jobName: "attendance-reminder",
    runKey: dateKey(date),
    metadata: { targetDate: dateKey(date) },
    handler: () => queueAttendanceReminders({ targetDate: date }),
  });
};
const runLeaveReminderJob = (targetDate) => {
  const date = resolveDate(targetDate, getBusinessDate(new Date()));
  return runScheduledJob({
    jobName: "leave-reminder",
    runKey: dateKey(date),
    metadata: { targetDate: dateKey(date) },
...
const schedule = (expression, name, handler) =>
  cron.schedule(
    ...
    () =>
      handler().catch((error) =>
        logger.error("Scheduled job execution failed", { jobName: name, error: error.message })
      ),
    { timezone }
  );
...
const startCronJobs = () => {
  ...
  const jobs = [
    schedule(
      process.env.ABSENCE_CRON_SCHEDULE || "15 0 * * *",
      "attendance-absence",
      runAttendanceAbsenceJob
    ),
    schedule(
      process.env.ATTENDANCE_REMINDER_CRON_SCHEDULE || "0 11 * * 1-5",
      "attendance-reminder",
      runAttendanceReminderJob
    ),
    schedule(
      process.env.LEAVE_REMINDER_CRON_SCHEDULE || "0 8 * * *",
      "leave-reminder",
      runLeaveReminderJob
    ),
    schedule(
...
const stopCronJobs = (jobs = []) => jobs.forEach((job) => job.stop());
