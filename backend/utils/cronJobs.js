...
const schedule = (expression, name, handler) => cron.schedule(expression, () => handler().catch((error) => logger.error("Scheduled job execution failed", { jobName: name, error: error.message })), { timezone });

const startCronJobs = () => {
  if (process.env.CRON_ENABLED === "false") { logger.info("Cron jobs are disabled"); return []; }
  const jobs = [
