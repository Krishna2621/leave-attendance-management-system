...
const allowed = (fields, label) =>
  body().custom((value, { req }) => {
    const unexpected = Object.keys(req.body).find((key) => !fields.includes(key));
    if (unexpected) throw new Error(`Field '${unexpected}' is not allowed for ${label}`);
    return true;
  });
const profileValidation = (includeJoining = false) => [
  ...
  body().custom((value, { req }) => {
    if (!Object.keys(req.body).length) throw new Error("At least one profile field is required");
    return true;
  }),
  body("name")
    .optional()
  ...
  body("emergencyContact.phoneNumber")
    .optional()
    .matches(/^\+?[1-9]\d{6,14}$/)
    .withMessage("Emergency contact phone number must be valid"),
];
const pagination = [
...
router.get(
  ...
  [
    ...
    query("isActive").optional().isBoolean({ strict: true }).toBoolean(),
...
router.put(
  "/:id",
  authorizeRoles("hr", "admin"),
  [param("id").isMongoId(), ...profileValidation(true)],
  validateRequest,
...
router.put(
  ...
  [
    param("id").isMongoId(),
    allowed(["isActive"], "status update"),
    body("isActive")
...
router.put(
  ...
  [
    param("id").isMongoId(),
    allowed(["managerId"], "manager assignment"),
    body("managerId")
      .optional({ nullable: true })
      .isMongoId()
...
router.put(
  ...
  [
    param("id").isMongoId(),
    allowed(["departmentId"], "department assignment"),
    body("departmentId")
      .optional({ nullable: true })
      .isMongoId()
