...
const validateEnvironment = () => {
  const missing = ["PORT", "MONGO_URI", "JWT_SECRET", "JWT_REFRESH_SECRET", "NODE_ENV"].filter(
  ...
  if (
    ...
    throw new Error(
      "Invalid environment configuration: PORT must be an integer between 1 and 65535."
    );
  }
  if (
  ...
  if (isSet(process.env.FRONTEND_URL)) {
    try {
      ...
      throw new Error("Invalid environment configuration: FRONTEND_URL must be a valid URL.");
    }
  }
  if (missing.length)
...
const hasCloudinaryConfig = () =>
  ["CLOUDINARY_CLOUD_NAME", "CLOUDINARY_API_KEY", "CLOUDINARY_API_SECRET"].every((name) =>
    isSet(process.env[name])
  );
