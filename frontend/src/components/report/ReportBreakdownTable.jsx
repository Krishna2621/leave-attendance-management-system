      ...
      {rows.length ? (
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((row) => (
                <tr key={accessor(row)._id || accessor(row).name} className="text-slate-700">
                  ...
                  <td className="py-2.5 pl-4 text-right">
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${row.attendanceRate >= 90 ? "bg-teal-50 text-teal-700" : row.attendanceRate >= 75 ? "bg-amber-50 text-amber-700" : "bg-rose-50 text-rose-700"}`}
                    >
                      {row.attendanceRate}%
                    </span>
                  </td>
        ...
        <div className="mt-4">
          <EmptyState
            ...
            description="There are no records for this grouping in the selected range."
          />
        </div>
      )}
    </Card>
