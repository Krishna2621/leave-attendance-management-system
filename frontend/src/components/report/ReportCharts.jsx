    ...
    {hasData ? (
      ...
      <div className="mt-5">
        <EmptyState
          ...
          description="Adjust the filters to see chart data."
        />
      </div>
    )}
  </Card>
...
export function StatusBreakdownChart({ summary }) {
  ...
  return (
    <ChartShell
      ...
      detail="Distribution of attendance records by status"
...
export function DepartmentAttendanceChart({ departmentWise }) {
  const data = (departmentWise || []).map((item) => ({
    name: item.department.name,
    rate: item.attendanceRate,
    late: item.lateArrivals,
  }));
  return (
    <ChartShell
      title="Attendance Rate by Department"
      detail="Attendance rate across departments in the selected range"
      hasData={data.length > 0}
    >
      <ResponsiveContainer width="100%" height="100%">
...
export function DailyTrendChart({ dailyTrend }) {
  const data = (dailyTrend || []).map((item) => ({
    name: new Date(`${item.date}T00:00:00.000Z`).toLocaleDateString(undefined, {
      month: "short",
      day: "numeric",
    }),
    rate: item.attendanceRate,
  ...
  return (
    <ChartShell
      ...
      detail="Daily attendance rate and late arrivals over time"
      hasData={data.length > 0}
    >
      <ResponsiveContainer width="100%" height="100%">
