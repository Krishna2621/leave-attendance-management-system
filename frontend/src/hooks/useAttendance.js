...
export function useAttendance(scope, params, enabled = true) {
  ...
  const backendParams =
    scope === "organization"
      ? params
      : Object.fromEntries(
          Object.entries(params).filter(([key]) => !["status", "isLate", "search"].includes(key))
        );
  return useQuery({
    queryKey: queryKey(scope, params),
    queryFn: () => fetcher(backendParams),
    enabled,
...
export function useAttendanceActions() {
  ...
  return {
    punchIn: useMutation({ mutationFn: punchIn, ...options }),
    punchOut: useMutation({ mutationFn: punchOut, ...options }),
    correct: useMutation({
