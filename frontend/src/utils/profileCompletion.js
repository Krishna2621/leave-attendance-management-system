...
export const profileCompletionFields = [
  {
    key: "phoneNumber",
    label: "Phone number",
    isComplete: (profile) => hasText(profile?.phoneNumber),
  },
  ...
  { key: "gender", label: "Gender", isComplete: (profile) => hasText(profile?.gender) },
  {
    ...
    isComplete: (profile) => hasText(profile?.emergencyContact?.name),
  },
  {
    key: "emergencyPhone",
    label: "Emergency phone",
    isComplete: (profile) => hasText(profile?.emergencyContact?.phoneNumber),
  },
];

export function getProfileCompletion(profile) {
  const fields = profileCompletionFields.map((field) => ({
    ...field,
