...
export default function ProfilePage() {
  ...
  const submit = async (values) => {
    ...
    if (values.emergencyName.trim() || values.emergencyPhone.trim())
    ...
    try {
      ...
      if (payload.name) updateUser({ name: data.user?.name || payload.name });
  ...
  const onPickFile = async (event) => {
    ...
    if (!validImageTypes.includes(file.type) || file.size > 5 * 1024 * 1024) {
      toast.error("Use a JPG, JPEG, or PNG image up to 5 MB.");
      return;
    }
    try {
      const data = await uploadPicture.mutateAsync(file);
  ...
  return (
    <div className="mt-6 space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">View and update your personal information.</p>
      </div>
      {completion.isComplete ? (
        <Card className="border-emerald-100 bg-emerald-50 p-5">
          <div className="flex gap-4">
            <div className="grid size-11 shrink-0 place-items-center rounded-full bg-emerald-100 text-emerald-700">
              <CheckCircle2 size={24} aria-hidden="true" />
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="font-semibold text-emerald-950">Profile Completed</h2>
                <Badge tone="success">✓ All required fields completed</Badge>
              </div>
              <p className="mt-1 text-sm font-medium text-emerald-800">
                Congratulations! Your employee profile is fully completed.
              </p>
              <p className="mt-3 text-sm leading-6 text-emerald-900/80">
                Thank you for keeping your information up to date. All required profile information
                has been provided.
              </p>
            </div>
          </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                {completion.completedRequiredFields} of {completion.totalRequiredFields} required
                fields completed
              </p>
            </div>
          ...
          <ul className="mt-4 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
            {completion.fields.map((field) => (
              <li
                key={field.key}
                className={`flex items-center gap-2 ${field.complete ? "text-emerald-700" : "text-slate-500"}`}
              >
                {field.complete ? (
                  <CheckCircle2 size={17} aria-hidden="true" />
                ) : (
                  <Circle size={17} className="text-rose-500" aria-hidden="true" />
                )}
                <span>{field.label}</span>
                <span className="sr-only">{field.complete ? "completed" : "pending"}</span>
              </li>
            ))}
          </ul>
        </Card>
          ...
          <div className="flex flex-col items-center text-center">
            <div className="relative">
              ...
              <button
                ...
                className="absolute bottom-0 right-0 grid size-8 place-items-center rounded-full bg-teal-700 text-white shadow hover:bg-teal-800 disabled:opacity-60"
            ...
            <div className="mt-3">
              <Badge tone={roleTone(profile.role)}>{roleText(profile.role)}</Badge>
            </div>
            ...
            <>
              <Card className="p-6">
                <div className="mb-2 flex items-center justify-between">
                  <h2 className="font-semibold text-slate-900">Personal & contact</h2>
                ...
                <div className="divide-y divide-slate-100">
                  ...
                  <Row label="Gender" value={genderText(profile.gender)} />
                  <Row label="Blood group" value={profile.bloodGroup} />
                  <Row label="Emergency contact" value={profile.emergencyContact?.name} />
                  <Row label="Emergency phone" value={profile.emergencyContact?.phoneNumber} />
                </div>
              </Card>
              <Card className="p-6">
                <h2 className="mb-2 font-semibold text-slate-900">Employment</h2>
                <div className="divide-y divide-slate-100">
                  <Row label="Role" value={roleText(profile.role)} />
                  <Row label="Department" value={profile.departmentId?.name} />
                  <Row label="Manager" value={profile.managerId?.name} />
                  <Row label="Joining date" value={formatDate(profile.joiningDate)} />
                </div>
              </Card>
            </>
          )}
