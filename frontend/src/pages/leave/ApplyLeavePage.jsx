...
export default function ApplyLeavePage() {
  ...
  const submit = async (values) => {
    if (file && (!validTypes.includes(file.type) || file.size > 10 * 1024 * 1024)) {
      toast.error("Use a PDF, JPG, JPEG, or PNG document up to 10 MB.");
      return;
    }
    try {
      await apply.mutateAsync({ values: { ...values, document: file }, onProgress: setProgress });
  ...
  return (
    <div className="mx-auto mt-6 max-w-2xl">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Submit a leave request with an optional supporting document.
      </p>
      <Card className="mt-6 p-6">
        <form className="space-y-5" onSubmit={handleSubmit(submit)}>
          ...
          >
            ...
            {(types.data?.leaveTypes || []).map((type) => (
