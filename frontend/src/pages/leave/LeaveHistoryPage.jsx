...
export default function LeaveHistoryPage() {
  ...
  const rows = useMemo(() => {
    ...
    return (requests.data?.leaveRequests || []).filter((request) => {
      ...
      const matchesSearch =
        !term ||
        `${request.leaveTypeId?.name || ""} ${request.reason} ${request.status}`
          .toLowerCase()
          .includes(term);
  ...
  return (
    <div className="mt-6 space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">View completed leave requests</p>
      </div>
      <section className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2">
        <label className="text-sm font-medium text-slate-700">
          ...
          <select
            ...
            className="mt-1.5 w-full rounded-lg border border-slate-300 p-2"
          >
        ...
        <label className="text-sm font-medium text-slate-700">
          ...
          <input
            ...
            placeholder="Leave type, reason, or status"
            className="mt-1.5 w-full rounded-lg border border-slate-300 p-2"
          />
        </label>
      </section>
      <Table
        ...
        data={rows}
        keyField="_id"
        emptyMessage="No completed leave requests found."
      />
      <LeaveDetailsModal
        request={selected}
