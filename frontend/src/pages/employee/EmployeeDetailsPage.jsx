...
export default function EmployeeDetailsPage() {
  ...
  const run = async (mutation, variables, message) => {
    try {
      await mutation.mutateAsync({ id, ...variables });
  ...
  return (
    <div className="mt-6 space-y-5">
      <button
        ...
        className="flex items-center gap-1 text-sm text-slate-500 hover:text-teal-700"
      >
        <ArrowLeft size={16} />
        Back to directory
      </button>
        ...
        <Card className="p-6 lg:col-span-1">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-3 flex gap-2">
              <Badge tone={roleTone(employee.role)}>{roleText(employee.role)}</Badge>
          ...
          {isManager && (
            <div className="mt-6 space-y-2 border-t border-slate-100 pt-5">
              <Button className="w-full" variant="secondary" onClick={() => setDialog("edit")}>
                Edit employee
              </Button>
              <Button
                className="w-full"
              ...
              <Button className="w-full" variant="secondary" onClick={() => setDialog("manager")}>
                Assign manager
              </Button>
              {isAdmin && (
                <Button className="w-full" variant="secondary" onClick={() => setDialog("role")}>
                  Change role
                </Button>
              )}
              {isAdmin && (
        ...
        <div className="space-y-5 lg:col-span-2">
          ...
          <Card className="p-6">
            ...
            <div className="divide-y divide-slate-100">
              ...
              <Row label="Emergency contact" value={employee.emergencyContact?.name} />
              <Row label="Emergency phone" value={employee.emergencyContact?.phoneNumber} />
            </div>
          </Card>
        </div>
      </div>
      ...
      {dialog === "edit" && (
        <EditEmployeeModal
          ...
          onSubmit={(payload) => run(actions.update, { payload }, "Employee updated successfully.")}
        />
      )}
      <AssignmentModal
        ...
        loading={actions.changeDepartment.isPending}
        onClose={() => setDialog(null)}
        onSubmit={(departmentId) =>
          run(actions.changeDepartment, { departmentId }, "Department updated successfully.")
        }
      />
      <AssignmentModal
        open={dialog === "manager"}
        ...
        options={(managers.data?.users || [])
          .filter((m) => m._id !== employee._id)
          .map((m) => ({ value: m._id, label: `${m.name} (${m.email})` }))}
        loading={actions.changeManager.isPending}
        onClose={() => setDialog(null)}
        onSubmit={(managerId) =>
          run(actions.changeManager, { managerId }, "Manager updated successfully.")
        }
      />
      <AssignmentModal
        open={dialog === "role"}
        ...
        loading={actions.changeRole.isPending}
        onClose={() => setDialog(null)}
        onSubmit={(role) => run(actions.changeRole, { role }, "Role updated successfully.")}
      />
      <ConfirmDialog
        open={dialog === "status"}
        ...
        onClose={() => setDialog(null)}
        onConfirm={() =>
          run(actions.setStatus, { isActive: !employee.isActive }, "Status updated successfully.")
        }
      />
    </div>
  );
