...
const pageTitle = {
  ...
  organization: "Organization Attendance",
};
...
export default function AttendanceListPage({ scope }) {
  ...
  const rows = useMemo(() => {
    ...
    const statusFiltered =
      filters.status && scope !== "organization"
        ? records.filter((row) => row.status === filters.status)
        : records;
    const searched = term
      ? statusFiltered.filter((row) =>
          `${row.employee?.name || ""} ${row.status} ${row.date}`.toLowerCase().includes(term)
        )
      : statusFiltered;
  ...
  const submitCorrection = async (payload) => {
    try {
      await correct.mutateAsync({
        ...
        payload: {
          ...payload,
          punchIn: new Date(payload.punchIn).toISOString(),
          punchOut: new Date(payload.punchOut).toISOString(),
        },
      });
  ...
  return (
    <div className="mt-6 space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">{pageTitle[scope]}</h1>
        <p className="mt-1 text-sm text-slate-600">
          Filter, sort, and inspect available attendance records.
        </p>
      </div>
      <AttendanceFilters filters={filters} onChange={setFilters} scope={scope} />
      <AttendanceTable
        records={rows}
        scope={scope}
        onDetails={setSelected}
        onCorrect={
          ["hr", "admin"].includes(user.role) && scope === "organization"
            ? setCorrecting
            : undefined
        }
        sort={sort}
      ...
      />
      <AttendanceDetailsModal record={selected} onClose={() => setSelected(null)} />
      <CorrectionModal
        record={correcting}
