...
import {
  useAttendanceReport,
  useReportDepartmentOptions,
  useReportEmployeeOptions,
...
export default function AttendanceReportPage() {
  ...
  return (
    <div className="mt-6 space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Analyze attendance across departments, managers, and time.
          </p>
        </div>
        {generatedAt && (
      ...
      {report.isLoading ? (
        <Loader label="Generating attendance report…" />
      ) : report.isError ? (
        <DashboardError error={report.error} onRetry={report.refetch} />
      ) : !hasRecords ? (
        <EmptyState
          title="No attendance records found"
          description="No records match the selected date range and filters. Try widening the range."
        />
      ) : (
        <>
