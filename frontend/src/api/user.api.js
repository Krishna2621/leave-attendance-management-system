...
export const getEmployees = (params) =>
  client.get("/users", { params: cleanParams(params) }).then(getData);
...
export const setEmployeeStatus = (id, isActive) =>
  client.put(`/users/${id}/status`, { isActive }).then(getData);
export const changeEmployeeRole = (id, role) =>
  client.put(`/users/${id}/role`, { role }).then(getData);
export const changeEmployeeManager = (id, managerId) =>
...
export const uploadMyProfilePicture = (file) => {
  ...
  return client
    .put("/users/me/profile-picture", form, { headers: { "Content-Type": "multipart/form-data" } })
    .then(getData);
};
